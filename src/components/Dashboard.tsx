...
export default function Dashboard({ profile, language }: DashboardProps) {
  ...
  useEffect(() => {
    async function fetchCareerInfo() {
      if (!selectedCareerTrend) {
        setCareerInsightData(null);
        return;
      }
  ...
  useEffect(() => {
    const q = query(
      collection(db, 'users', profile.uid, 'recommendations'),
  ...
  // Auto-refresh recommendations when profile or language changes
  useEffect(() => {
    if (recommendations.length > 0 && !isGenerating) {
      ...
      // Refresh if profile updated OR language changed
  ...
  // Auto-analyze trends when data is ready
  useEffect(() => {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 px-6 py-3 bg-[#ff4e00] hover:bg-[#ff6a2a] rounded-2xl font-bold transition-all disabled:opacity-50 w-full sm:w-auto"
        >
        ...
        <div className="lg:col-span-2 bg-white/5 rounded-3xl border border-white/10 p-4 sm:p-6 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                disabled={isAnalyzing || isLoadingTrends}
                className="text-[10px] sm:text-xs text-[#ff4e00] font-bold uppercase tracking-widest bg-[#ff4e00]/10 px-3 py-1.5 rounded-lg hover:bg-[#ff4e00]/20 transition-all flex items-center gap-2"
              >
                ...
                <AreaChart data={marketTrends}>
                  ...
                  {marketTrends.length > 0 && Object.keys(marketTrends[0]).filter(k => k !== 'year').map((career, idx) => (
                    <Area 
                      ...
                      type="monotone" 
                      dataKey={career} 
                      stroke={idx === 0 ? '#ff4e00' : `hsl(${idx * 40}, 70%, 50%)`} 
                      fillOpacity={1} 
                      fill="url(#colorScore)" 
                    />
                  ))}
          ...
          {trendAnalysis && (
            <motion.div 
              ...
              className="p-4 bg-[#ff4e00]/5 border border-[#ff4e00]/20 rounded-2xl text-sm text-gray-300 leading-relaxed relative group"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2 text-[#ff4e00] font-bold uppercase text-xs">
                  <Sparkles className="w-4 h-4" />
                  {t('aiTrendInsight', language)}
                </div>
                <button 
                  onClick={() => speakText(trendAnalysis, language)}
                  className="p-1.5 bg-white/5 hover:bg-white/10 rounded-lg text-gray-400 hover:text-[#ff4e00] transition-all"
        ...
        <div className="bg-white/5 rounded-3xl border border-white/10 p-6 space-y-6">
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-[#ff4e00] font-bold uppercase text-xs">
                  ...
                  {t('emergingProfessions', language) || 'Emerging Horizons'}
                </div>
                <div className="space-y-2">
                  {rawMarketTrends.length > 0 && rawMarketTrends.map((career: any, i: number) => (
                    <button 
                      key={career.name} 
                      onClick={() => setSelectedCareerTrend(career.name)}
                      className="w-full flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 hover:border-[#ff4e00]/30 transition-all group text-left"
                    >
                      <div className="flex items-center gap-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4">
                    <p className="text-xs text-gray-400 italic">
                      {t('marketRiskAnalysisDesc', language)}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="space-y-6">
        ...
        {recommendations.length === 0 && !isGenerating ? (
          <div className="p-12 text-center bg-white/5 rounded-3xl border border-dashed border-white/20">
            ...
            <button 
              onClick={handleGenerateRecs}
              className="px-6 py-3 bg-white/10 hover:bg-white/20 rounded-2xl transition-all"
            >
              {t('generateNow', language)}
            </button>
          </div>
        ) : (
              ...
              {isGenerating ? (
                ...
                [1, 2, 3].map((_, i) => (
                  <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="bg-white/5 rounded-3xl border border-white/10 p-6 space-y-4 animate-pulse"
                  >
                ...
                recommendations.slice(0, 3).map((rec, i) => (
                  <motion.div 
                    ...
                    className="bg-white/5 rounded-3xl border border-white/10 p-6 space-y-4 hover:bg-white/10 transition-all group"
                  >
                    <div className="flex justify-between items-start">
                      <div className="w-12 h-12 bg-[#ff4e00]/20 rounded-2xl flex items-center justify-center text-[#ff4e00] font-bold text-xl">
                        {rec.matchScore}%
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <div className="text-xs text-green-500 font-bold uppercase tracking-widest">{t('highMatch', language)}</div>
                        <div className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${
                          rec.riskLevel === 'low' ? 'border-green-500 text-green-500' : 
                          rec.riskLevel === 'medium' ? 'border-yellow-500 text-yellow-500' : 
                          'border-red-500 text-red-500'
                        }`}>
                          {t(rec.riskLevel || 'low', language)} {t('risk', language)}
                        </div>
                        <div className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border border-gray-500 text-gray-400`}>
                          {rec.futureDemandTrend || t('stable', language)}
                        </div>
                      </div>
                    </div>
                    <div>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {rec.skillGap?.map(skill => (
                            <span key={skill} className="px-2 py-1 bg-white/5 rounded-lg text-[10px] border border-white/10">{skill}</span>
                          ))}
                        </div>
                      </div>
                      ...
                      {rec.suggestedUniversities && rec.suggestedUniversities.length > 0 && (
                        <div className="space-y-2">
                          <div className="text-xs font-bold text-[#ff4e00] uppercase flex items-center gap-1">
                            ...
                            {t('suggestedUniversities', language)}
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {rec.suggestedUniversities.map(uni => (
                              <span key={uni} className="px-2 py-1 bg-[#ff4e00]/5 rounded-lg text-[10px] border border-[#ff4e00]/10 text-gray-300">{uni}</span>
                            ))}
                          </div>
                        </div>
                      )}
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="space-y-1">
                          {rec.roadmap?.slice(0, 3).map((step, idx) => (
                    ...
                    <button 
                      onClick={() => setSelectedRoadmap(rec)}
                      className="w-full py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all"
                    >
                      {t('viewRoadmap', language)}
        ...
        {selectedRoadmap && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div 
              ...
              className="bg-[#0f0a07] border border-white/10 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
            >
              ...
              <div className="flex-1 overflow-y-auto p-6 sm:p-8 space-y-8">
                <div className="relative">
                  ...
                  <div className="space-y-8">
                    {selectedRoadmap.roadmap?.map((step, idx) => (
                      ...
                      >
                        <div className="absolute left-0 top-0 w-8 h-8 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center font-bold text-[#ff4e00] shadow-xl z-10 transition-transform group-hover:scale-110">
                          {idx + 1}
                        </div>
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-4 hover:border-[#ff4e00]/30 transition-all">
                ...
                <div className="p-4 bg-[#ff4e00]/5 border border-[#ff4e00]/20 rounded-2xl">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-xs uppercase text-[#ff4e00] mb-1">{t('proTip', language) || 'Pro Tip'}</h4>
                      <p className="text-[11px] sm:text-xs text-gray-400 leading-relaxed italic">
                        {selectedRoadmap.proTip || (
                          language === 'Uzbek' ? "Insonlar bilan muloqot va ijodiy fikrlash bu yo'lda sizga eng katta yordamchi bo'ladi. Hech qachon o'rganishdan to'xtamang." : 
                          language === 'Turkish' ? "İletişim ve yaratıcı düşünme bu yolda size en büyük yardımcı olacaktır. Öğrenmeyi asla bırakmayın." :
                          "Networking and creative thinking will be your best allies. Never stop learning as the field evolves rapidly."
                        )}
                      </p>
                    </div>
                  </div>
              ...
              {selectedRoadmap.suggestedUniversities && selectedRoadmap.suggestedUniversities.length > 0 && (
                <div className="p-6 bg-white/5 border-t border-white/10">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedRoadmap.suggestedUniversities.map(uni => (
                      <button 
                        key={uni} 
                        onClick={() => setSelectedUniversityName(uni)}
                        className="px-3 py-1.5 bg-white/5 hover:bg-[#ff4e00]/10 hover:border-[#ff4e00]/30 rounded-lg text-xs border border-white/10 font-medium transition-all"
                      >
                        {uni}
                      </button>
                    ))}
        ...
        {selectedCareerTrend && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            <motion.div 
              ...
              className="bg-[#0f0a07] border border-white/10 w-full max-w-xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]"
            >
              <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5">
                <h3 className="font-bold text-sm uppercase tracking-widest text-[#ff4e00]">{t('careerInsights', language)}</h3>
                <button 
                  onClick={() => {
                    setSelectedCareerTrend(null);
                    setCareerInsightData(null);
                  }}
                ...
                {isLoadingCareerInsight ? (
                  ...
                  <div className="p-6 sm:p-8 space-y-6">
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-4 text-xs font-semibold uppercase tracking-wider text-gray-400">
                        <div className="flex items-center gap-1.5 bg-white/5 px-2.5 py-1 rounded-lg">
                          ...
                          {careerInsightData.demandLevel} Demand
                        </div>
                        <div className="flex items-center gap-1.5 bg-white/5 px-2.5 py-1 rounded-lg">
                          <Target className="w-3.5 h-3.5 text-yellow-500" />
                          {careerInsightData.salaryRange}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="aspect-video w-full bg-white/5 rounded-2xl overflow-hidden border border-white/10 relative group">
                      <img 
                        src={`https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?auto=format&fit=crop&q=80&w=800&q=80`} 
                        alt="Career Growth" 
                    ...
                    <div className="space-y-4">
                      <div>
                        <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">Overview</h4>
                        <p className="text-gray-300 leading-relaxed text-sm sm:text-base">
                          {careerInsightData.description}
                        </p>
                      </div>

                      <div className="p-4 bg-[#ff4e00]/5 border border-[#ff4e00]/20 rounded-2xl">
                        <h4 className="text-xs font-bold text-[#ff4e00] uppercase mb-2 flex items-center gap-2">
                          <Sparkles className="w-3 h-3" />
                          {t('futureGrowth', language)}
                        </h4>
                        <p className="text-xs text-gray-400 leading-relaxed">
                          {careerInsightData.futureGrowth}
                        </p>
                      </div>
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-gray-500 uppercase mb-3 flex items-center gap-2">
                          ...
                          {t('skillsRequired', language)}
                        </h4>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {careerInsightData.skills.map((skill: string, idx: number) => (
                            <div key={idx} className="flex items-center gap-3 p-2.5 bg-white/5 rounded-xl border border-white/10 text-xs text-gray-300 font-medium">
                              {skill}
                            </div>
                          ))}
                        </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {isLoadingUni ? (
                  ...
                  <div className="p-6 sm:p-8 space-y-6">
                    ...
                    <div className="aspect-video w-full bg-white/5 rounded-2xl overflow-hidden border border-white/10 relative group">
                      <img 
                        src={`https://images.unsplash.com/photo-1541339907198-e08756ebafe3?auto=format&fit=crop&q=80&w=800&q=80`} 
                      ...
                      <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-black/40 backdrop-blur-sm px-3 py-1.5 rounded-xl border border-white/10">
                        ...
                        <span className="text-[10px] uppercase font-bold text-white/70">Representational AI Image</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {universityData.keyFeatures.map((feature: string, idx: number) => (
                        <div key={idx} className="flex items-start gap-3 p-3 bg-white/5 rounded-xl border border-white/10">
                          <div className="w-6 h-6 rounded-lg bg-[#ff4e00]/10 flex items-center justify-center text-[#ff4e00] shrink-0 mt-0.5">
                    ...
                    <a 
                      href={universityData.searchLink} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="w-full py-4 bg-[#ff4e00] hover:bg-[#ff6a2a] text-white rounded-2xl font-bold flex items-center justify-center gap-2 transition-all shadow-xl shadow-[#ff4e00]/10"
                    >
